...
export const getCurrentGame = createSelector(
  ...
  (games, gameId, users) =>
    getGameWithRelationships(games.find(game => game.id === gameId), { users })
);
...
export const getCurrentPlayerFromCurrentGame = createSelector(
  ...
  (currentGame, user) =>
    currentGame &&
    currentGame._players &&
    user &&
    user.id &&
    currentGame._players.find(player => player.id === user.id)
);
...
export const getNewGames = createSelector(
  ...
  (games, currentGameId) =>
    games.filter(
      game =>
        game.createdRealtime && !game.createdByMe && game.id !== currentGameId
    )
);
