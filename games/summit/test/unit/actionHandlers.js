...
tape('doAction.swap', t => {
  ...
  t.equals(
    game._players[2]._hidden.role,
    'center0',
    'Role was swapped with center'
  );
  t.equals(game._players[3]._hidden.role, 'test0', 'Role was swapped out');
  t.equals(
    game._players[0]._private.peekedOrMovedCards,
    true,
    'peekedOrMovedCards flag set'
  );
  t.equals(
    game._players[2]._private.peekedOrMovedCards,
    true,
    'peekedOrMovedCards flag set'
  );
  ...
  game._players.forEach((p, i) => {
    ...
    if (p.id !== 'p0' && p.id !== 'p2') {
      t.equals(
        p._private.peekedOrMovedCards,
        undefined,
        'peekedOrMovedCards flag not set'
      );
    }
  });
...
const rolesThatRequireTargets = ['doppleganger', 'alphaWolf', 'drunk'];
tape('Roles that require targets throw when not provided one', t => {
  ...
  rolesThatRequireTargets.forEach(handlerName => {
    t.throws(
      ...
      Error,
      `${handlerName} throws when no target provided`
    );
  });
  t.end();
});
...
tape('Roles that cannot target themselves throw when it happens', t => {
  ...
  rolesThatCannotSelfTarget.forEach(handlerName => {
    t.throws(
      ...
      Error,
      `${handlerName} throws when targeting self`
    );
  });
  t.end();
});
...
tape('doppleganger', t => {
  const descriptor = [
    {
      kind: 'N',
      path: ['_players', 1, '_private', 'peekedOrMovedCards'],
      rhs: true
    },
    {
      kind: 'N',
      path: ['_players', 1, '_private', 'peeked', 'p3'],
      rhs: 'test3'
    },
    {
      kind: 'N',
      path: ['_players', 1, '_private', 'doppleganger'],
      rhs: 'test3'
    },
...
tape('sentinel', t => {
  standardP1toP3(t, 'sentinel', [
    { kind: 'N', path: ['_players', 3, 'shield'], rhs: true }
  ]);
});
