...
function getNewGame(villagers, werewolves, roles) {
  ...
  game.players = _.times(
    villagers + werewolves + Object.keys(game.options).length - 5,
    i => 'id' + i
  );
  game._players = _.times(
    villagers + werewolves + Object.keys(game.options).length - 5,
    i => ({
      id: 'id' + i,
      _hidden: {},
...
tape.skip('setup', t => {
  ...
  summit.troublemaker(
    { target1: 'id1', target2: 'id2', user: { id: 'id0' } },
    game
  );
  t.equals(game.mode, 'troublemaker');
  summit.troublemaker(
    { target1: 'id0', target2: 'id2', user: { id: 'id1' } },
    game
  );
  t.equals(game.mode, 'troublemaker');
  summit.troublemaker(
    { target1: 'id1', target2: 'id3', user: { id: 'id2' } },
    game
  );
  t.equals(game.mode, 'troublemaker');
  summit.troublemaker(
    { target1: 'id1', target2: 'id2', user: { id: 'id3' } },
    game
  );
