...
export default class Board extends Component {
  ...
  componentWillUpdate(nextProps) {
    if (
      this.props.players.length !== nextProps.players.length ||
      this.props.game.centerCount !== nextProps.game.centerCount
    ) {
      this.playerStyles = this.buildPlayerStyles();
    }
  }
          ...
          {players.map((player, i) => {
            ...
            return (
              <Player
                ...
                role={
                  player.role ||
                  me._private.peeked[player.id] ||
                  (isSelf && player._private.role)
                }
  ...
  buildPlayerStyles() {
    ...
    return times(this.props.players.length, i => ({
      ...
      height: itemSize,
      margin: -(itemSize / 2) - 6,
      transform: `rotate(${angle * i}deg) translate(${circleSize /
        2}${unit}) rotate(-${angle * i}deg)`
    }));
  }
}
