...
export default class PlayArea extends Component {
  ...
  render() {
    ...
    const progress =
      (game.order.indexOf(game.mode) + 1 / game.order.length) * 100;

    const itsMyTurn =
      !me.ready &&
      (game.mode === me._private.role ||
        (['paranormalInvestigator', 'witch'].includes(me._private.role) &&
          game.mode.indexOf(me._private.role) === 0) ||
        (me._private.role === 'doppleganger' &&
          game.mode.indexOf('doppleganger' + me._private.doppleganger) === 0));
    ...
    if (game.mode === 'day') {
      ...
      if (!game.winners.length) {
        ...
        const winnersNames = game.winners.map(
          winnerId => game._players.find(player => player.id === winnerId).name
        );
      ...
      if (!game.deadPlayers.length) {
        ...
        const deadPlayersNames = game.deadPlayers.map(
          player => game._players.find(p => p.id === player.id).name
        );
    ...
    const readyButton = !actionNeeded &&
      !me.ready &&
      game.mode !== 'day' &&
      game.mode !== 'gameover' && (
        <FlatButton
          ...
          label={
            secondsLeft < 0
              ? 'Ready'
              : moment.duration(secondsLeft, 's').humanize()
          }
          primary={true}
    ...
    const leftAndRightButtons = actionNeeded &&
      actionNeeded.chooseLeftOrRight && [
        <FlatButton
  ...
  onClickCard(actionNeeded, card) {
    ...
    if (!actionNeeded) {
      ...
      (typeof actionNeeded.player === 'boolean' && !isCenter) ||
      (typeof actionNeeded.center === 'boolean' && isCenter)
    ) {
      data = { target: card };
    } else if (
      typeof actionNeeded.player === 'number' ||
      typeof actionNeeded.center === 'number'
    ) {
      if (
        this.state.currentTarget &&
        (isCenter
          ? this.state.currentTarget.toString().length === 1
          : this.state.currentTarget.toString().length > 1)
      ) {
        data = {
