...
const doAction = function(game, action) {
  ...
  const getPlayer = idOrPlayer => {
    return typeof idOrPlayer === 'string'
      ? game._players.find(p => p.id === idOrPlayer)
      : idOrPlayer;
  };
  ...
  this.swap = () => {
    tasks.push(() => {
      if (players.length > 1) {
        ...
        let next,
          last = unshieldedPlayers[unshieldedPlayers.length - 1]._hidden.role;
  ...
  this.peek = cb => {
    ...
    tasks.push(() => {
      ...
      players
        ...
        .forEach(
          player =>
            (mainPlayer._private.peeked[player.id] = peekedAt =
              player._hidden.role)
        );

      centers.forEach(
        center =>
          (mainPlayer._private.peeked[center.toString()] = peekedAt =
            game._hidden.center[center])
      );
...
const handlers = {
  doppleganger(game, pick) {
    ...
    doAction(game, pick)
      ...
      .peek(
        (player, peekedAt) =>
          (player._private.doppleganger = game._hidden.doppleganger = peekedAt)
      )
      .do();
  },
  sentinel(game, pick) {
  ...
  thing(game, pick) {
    ...
    let currentPlayerIndex = _.findIndex(
      game._players,
      player => pick.user.id === player.id
    );
    let pickedPlayerIndex = _.findIndex(
      game._players,
      player => pick.user.id === pick.target
    );

    if (
      pickedPlayerIndex !== currentPlayerIndex - 1 ||
      pickedPlayerIndex !== currentPlayerIndex + 1 ||
      (currentPlayerIndex === 0 &&
        pickedPlayerIndex !== game._players.length - 1) ||
      (currentPlayerIndex === game._players.length - 1 &&
        pickedPlayerIndex !== 0)
    ) {
      throwFatalError(pick);
    }
  ...
  seer(game, pick) {
    if (
      (!_.isNil(pick.target1) && _.isNil(pick.target2)) ||
      (_.isNil(pick.target1) && !_.isNil(pick.target2)) ||
      pick.target === pick.user.id
    ) {
      throwFatalError(pick);
    }
  ...
  paranormalInvestigator2(game, pick) {
    ...
    forPlayer(
      pick.user.id,
      ({ _private: { werewolf, tanner, vampire } }) => {
  ...
  auraSeer(game, pick) {
    const auraPlayers = game._players.filter(
      player => player._private.peekedOrMovedCards
    );
    forPlayer(
      pick.user.id,
      player =>
        (player._hidden.knows = player._hidden.knows.concat(auraPlayers)),
      game
    );
  },
  ...
  squire(game, pick) {
    ...
    game._players
      .filter(player =>
        playerThinksTheyAre(player, WEREWOLF_ROLES_THAT_ARE_KNOWN)
      )
  ...
  revealer(game, pick) {
    ...
    forPlayer(
      ...
      target => {
        ...
        if (
          target._hidden.role !== 'werewolf' &&
          target._hidden.role !== 'tanner'
        ) {
  ...
  curator(game, pick) {
    const artifactsInUse = game._players.map(
      player => player._private.artifact
    );
    ...
    if (pick.target) {
      ...
      forPlayer(
        ...
        player => {
          ...
          if (
            ['werewolf', 'villager', 'vampire']
              .concat(roles.map(role => role.name))
              .includes(player._private.artifact)
          ) {
            player._hidden.ignoredRole = player._hidden.role;
            player._hidden.role = player._private.role =
              player._private.artifact;
          }
        },
        game
      );
...
handlers.dopplegangerapprenticeSeer = {
  dopplegangerOf: handlers.apprenticeSeer
};
handlers.dopplegangerparanormalInvestigator = {
  dopplegangerOf: handlers.paranormalInvestigator,
  intermediateRole: 'dopplegangerparanormalInvestigator2'
};
handlers.dopplegangerparanormalInvestigator2 = {
  dopplegangerOf: handlers.paranormalInvestigator2,
  preIntermediateRole: 'dopplegangerparanormalInvestigator'
};
handlers.dopplegangerrobber = { dopplegangerOf: handlers.robber };
handlers.dopplegangerwitch = {
  dopplegangerOf: handlers.witch,
  intermediateRole: 'dopplegangerwitchSwaps'
};
handlers.dopplegangerwitchSwaps = {
  dopplegangerOf: handlers.witchSwaps,
  preIntermediateRole: 'dopplegangerwitch'
};
