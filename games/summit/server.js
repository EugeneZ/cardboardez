...
const WEREWOLF_ROLES_THAT_ARE_KNOWN = [
  'werewolf',
  'alphaWolf',
  'mysticWolf',
  'dreamWolf'
...
function playerIncludesRolesAt(player, location, roleOrRoles, game) {
  ...
  return (
    roles.includes(player[location].role) ||
    (player[location].role === 'doppleganger' &&
      roleOrRoles.includes(
        game ? game[location].doppleganger : player[location].doppleganger
      )) ||
    // TODO: FIX THIS! Pi works differently now
    (roles.includes('werewolf') &&
    player[location].role === 'paranormalInvestigator' &&
    game
      ? game[location].piIsWerewolf
      : player[location].piIsWerewolf) ||
    (roles.includes('tanner') &&
    player[location].role === 'paranormalInvestigator' &&
    game
      ? game[location].piIsTanner
      : player[location].piIsTanner) ||
    (roles.includes('vampire') &&
    player[location].role === 'paranormalInvestigator' &&
    game
      ? game[location].piIsVampire
      : player[location].piIsVampire)
  );
}
...
function setKnows(players) {
  const werewolfRolesThatSeeKnowns = [
    'werewolf',
    'alphaWolf',
    'mysticWolf',
    'minion',
  ...
  players.forEach(
    player =>
      (player._private.knows = player._private.knows.filter(
        id => player.id !== id
      ))
  );
}
...
module.exports.setup = function(game) {
  ...
  game.ends = moment()
    .add(game.minutes, 'minutes')
    .add(5, 'seconds')
    .unix(); //adding five seconds for lag or w/e
...
module.exports.night = function(action, game) {
  ...
  forPlayer(
    ...
    player => {
      ...
      if (isForDoppleganger) {
        thisPlayersTurn =
          player._private.role === 'doppleganger' &&
          game._hidden.doppleganger === dopplegangerOf;
      } else {
  ...
  if (game._players.every(p => p.ready)) {
    ...
    if (intermediateRole) {
      ...
      game.role =
        game.order[game.order.indexOf(preIntermediateRole || role) + 1];
    }
...
module.exports.day = function(vote, game) {
  ...
  if (
    game._players.every(p => p.vote) &&
    (!game.ends || moment().unix() > game.ends)
  ) {
    ...
    const deadPlayers = game._players
      ...
      .filter(
        (player, i, sorted) =>
          player.votes > 1 && player.votes === sorted[0].votes
      );
    ...
    game._players.forEach(player => {
      ...
      if (player.vote && playerActuallyIs(player, 'hunter', game)) {
        deadPlayers.push(
          game._players.find(
            target => target.id === player.vote && !target.guarded
          )
        );
      }
    });
    ...
    game._players.forEach(player => {
      if (
        playerActuallyIs(player, ['tanner', 'apprenticeTanner'], game) &&
    ...
    if (!game.winners.length) {
      const werewolvesInPlay = game._players.filter(player =>
        playerActuallyIs(
          player,
          ['werewolf', 'alphaWolf', 'mysticWolf', 'dreamWolf'],
          game
        )
      );
      const minionSquiresInPlay = game._players.filter(player =>
        playerActuallyIs(player, ['minion', 'squire'], game)
      );
      const evilTeamInPlay = game._players.filter(player =>
        playerActuallyIs(
          player,
          [
            'werewolf',
      ...
      let goodTeamInPlay = game._players.filter(
        player =>
          !playerActuallyIs(
            player,
            [
      ...
      goodTeamInPlay
        .filter(player => playerActuallyIs(player, 'cursed', game))
        .forEach(cursed => {
          if (cursed.votes) {
            werewolvesInPlay
              .filter(p => playerActuallyIs(p, 'werewolf', game))
              .forEach(wolf => {
